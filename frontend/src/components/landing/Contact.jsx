...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(language === 'en' ? 'Something went wrong. Please try again.' : 'Algo correu mal. Por favor tente novamente.');
    } finally {
            ...
            <div>
              <form onSubmit={handleSubmit} className="space-y-5" data-testid="contact-form">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <Select value={formData.service} onValueChange={(val) => handleChange('service', val)}>
                      <SelectTrigger 
                        className="h-12 bg-white/[0.06] border-white/10 text-white focus:border-[#1A8A84] focus:ring-[#1A8A84]/20 rounded-xl text-sm [&>span]:text-white/20"
                ...
                <Button 
                  ...
                  className="w-full bg-white hover:bg-white/90 text-[#1A1A1A] h-13 text-sm font-semibold rounded-full transition-all duration-300 disabled:opacity-50"
                >
                  {isSubmitting ? (
                  ...
                  {isSubmitting ? (language === 'en' ? 'Sending...' : 'A enviar...') : t.form.submit}
                </Button>

                {/* WhatsApp button */}
                <div className="pt-4">
                  <a 
                    href="https://wa.me/351934229144" 
                    target="_blank" 
                    rel="noopener noreferrer"
                    data-testid="whatsapp-link"
                    className="flex items-center gap-3 p-4 rounded-xl border border-white/10 hover:border-[#25D366]/40 hover:bg-white/[0.04] transition-all duration-300 group"
                  >
