      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex items-center justify-between h-20">
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={cn(
                  ...
                  scrolled ? 'text-[#5C5C5C] hover:text-[#1A1A1A]' : 'text-white/70 hover:text-white'
                )}
              >
                {t[link]}
              </button>
          ...
          <div className="hidden md:flex items-center gap-4">
            <button
              ...
              className={cn(
                ...
                scrolled ? 'text-[#5C5C5C] border-[#E0DDD6] hover:border-[#1A8A84]' : 'text-white/60 border-white/20 hover:border-white/40'
              )}
            >
              {language === 'en' ? 'PT' : 'EN'}
            </button>
          ...
          <div className="md:hidden flex items-center gap-3">
            <button onClick={toggleLanguage} className={cn('text-[13px] font-semibold px-2 py-1', scrolled ? 'text-[#5C5C5C]' : 'text-white/60')}>
              {language === 'en' ? 'PT' : 'EN'}
            </button>
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              <SheetTrigger asChild>
                <button className={cn('p-2', scrolled ? 'text-[#1A1A1A]' : 'text-white')}>
